...
%     iname={ilist.name};
    for y=7:12
        save_folder_= char(strcat('C:\Users\DHRITIMAAN DAS\Desktop\Mapped_PEI\',num2str(L),'\',fName1(y),'\'));
                c=0;
                e=0;
                f=0;
                J=0;
            w=[];
            weights=[];
        ilist=dir(char(strcat(path,fName(i),'\',fName1(y),'\')));
        iname={ilist.name};
        for j=3:length(iname)
            %image path
            ipath=char(strcat(path,fName(i),'\',fName1(y),'\',iname(j)));
       ...
       %Finally Averaging all the Frames mapped to same PEI.
            for g=1:length(P)
                if P(g)<=length(iname)-2
                    ipath=char(strcat(path,fName(i),'\',fName1(y),'\',iname(P(g)+2)));
                    img=double(imread(ipath));
                    img_pei1=img_pei1+img;
                    c=c+1;
                end
                if P(g)>length(iname)-2 && P(g)<=2*(length(iname)-2)
                    ipath=char(strcat(path,fName(i),'\',fName1(y),'\',iname(P(g)-(length(iname)-2)+2)));
                    img=double(imread(ipath));
                    img_pei2=img_pei2+img;
                    u=u+1;
                end
                if P(g)>2*(length(iname)-2) && P(g)<=3*(length(iname)-2)
                    ipath=char(strcat(path,fName(i),'\',fName1(y),'\',iname(P(g)-2*(length(iname)-2)+2)));
                    img=double(imread(ipath));
                    img_pei3=img_pei3+img;
                    e=e+1;
                end
                if P(g)>3*(length(iname)-2) && P(g)<=4*(length(iname)-2)
                    ipath=char(strcat(path,fName(i),'\',fName1(y),'\',iname(P(g)-3*(length(iname)-2)+2)));
                    img=double(imread(ipath));
